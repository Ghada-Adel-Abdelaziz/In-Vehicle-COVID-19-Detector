/*
 * File Name:stm32f407xx_USART_Driver.c
 *Description: Source file for the UART Stm32f407 driver
...
brief -  This function enables or disables the given USART
Parameter  -  Base address of the USART peripheral(pointer to Register)
Parameter  -  Enable or disable macros
return     -
Note       =uint8_t--->unsigned integer type with width of exactly 8bits 
*/
void USART_ENABLE(USART_RegDef_t *pUSARTx, uint8_t Cmd) //DONE1
{
	if(Cmd == ENABLE)
	{
		pUSARTx->USART_CR1 |= (1 << USART_CR1_UE); //Set UE(USART ENABLE) bit in Register CR1
	}
	else
	{
		pUSARTx->USART_CR1 &= ~(1 << USART_CR1_UE); //Clear UE(USART ENABLE) bit in Register CR1
	}
}
void USART_PeriClockControl(USART_RegDef_t *pUSARTx, uint8_t EnCLK)//DONE2
{
	  if(EnCLK == ENABLE)
	   {
		   if(pUSARTx == USART1)
...
fn   -   USART_PeripheralControl
Parameter  -  Base address of the USART peripheral(pointer to Register)
Parameter  - Number of Flag that i want to check status
return     -uint8_t--->unsigned integer type with width of exactly 8bits
Note       = 
*/
//
...
void USART_SetBaudRate(USART_RegDef_t *pUSARTx, uint32_t BaudRate)//DONE3
{
