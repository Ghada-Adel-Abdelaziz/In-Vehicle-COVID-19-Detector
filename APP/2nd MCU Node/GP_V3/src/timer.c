...
void PWM_Set_Duty(uint8_t TIM_ID, uint8_t duty_cycle)
{
	TIM_RegDef_t *pTIMx;
	pTIMx = Timer_Arr[TIM_ID];
	...
	TIM_Get_Period(TIM_ID , &period);

	pTIMx->CCR1 = ( (duty_cycle * period) / 100 );
}

void TIM_Get_Period(uint8_t TIM_ID , uint32_t *Timer_Period)
{
	TIM_RegDef_t *pTIMx;
	pTIMx = Timer_Arr[TIM_ID];
